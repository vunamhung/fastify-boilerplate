import { zm } from './type';
import { isValidObjectId, Schema, SchemaTypes, Types } from 'mongoose';
import {
  z,
  ZodAny,
  ZodArray,
 ...
 */
export function zodSchema<T extends ZodRawShape>(schema: ZodObject<T>): Schema<z.infer<typeof schema>> {
 ...
 * });
 */
export const zUUID = z.string().uuid({ message: 'Invalid UUID' }).or(z.instanceof(Types.UUID).describe('UUID'));
...
function parseField<T>(
  ...
  if (field.description?.startsWith('ObjectId')) {
    ...
    const autopopulate = field.description.split(':')[2];
    if (ref && autopopulate) return parseObjectIdRef(required, ref, Boolean(autopopulate));
...
function parseNumber(field: ZodNumber, required: boolean = true, def?: number, unique: boolean = false, validate?: (v: number) => boolean): zm.mNumber {
  if (validate) {
    return {
      type: Number,
      default: def,
...
function parseEnum(values: string[], required: boolean = true, def?: string): zm.mString {
  return {
    type: String,
    default: def,
    enum: values,
...
function parseObjectIdRef(required: boolean = true, ref: string, autopopulate?: boolean): zm.mObjectId {
  if (autopopulate) return { type: SchemaTypes.ObjectId, ref, required, autopopulate };
  return { type: SchemaTypes.ObjectId, ref, required };
}
...
function parseMixed(required: boolean = true, def?: unknown): zm.mMixed<unknown> {
  return {
